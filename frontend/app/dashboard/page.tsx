		...
		const loadData = async () => {
			...
			try {
				...
				if (user.role === "HR") {
					...
					const teamAppraisalsRes = await apiClient.getAppraisalsByManager(
						user.id,
					);
					if (teamAppraisalsRes.success && teamAppraisalsRes.data) {
						const goalsResponses = await Promise.all(
							teamAppraisalsRes.data.map((a) =>
								apiClient.getGoalsByAppraisal(a.id),
							),
						);
					...
					if (goalsRes.success && goalsRes.data) {
						...
						setGoalTotals({
							...
							completed: goalsRes.data.filter((g) => g.status === "COMPLETED")
								.length,
						});
					} else {
						setGoalTotals({ total: 0, completed: 0 });
		...
		<div className="space-y-8">
			...
			<div
				className={`grid gap-4 md:grid-cols-2 ${
					user.role === "HR" ? "lg:grid-cols-3" : "lg:grid-cols-4"
				}`}
			>
				{stats.map((stat) => (
					<Card key={stat.label}>
