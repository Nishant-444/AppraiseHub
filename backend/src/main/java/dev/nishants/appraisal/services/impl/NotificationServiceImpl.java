...
public class NotificationServiceImpl implements NotificationService {
  ...
  private void sendEmailForType(User user, String title, String message, Type type,
    ...
    String htmlBody = switch (type) {
      case CYCLE_STARTED -> emailTemplateService.cycleStarted(
              user.getFullName(),
              cycleName,
              startDate,
              endDate);
      case SELF_ASSESSMENT_SUBMITTED -> emailTemplateService.selfAssessmentSubmitted(
              user.getFullName(),
              employeeName,
              cycleName);
      case MANAGER_REVIEW_DONE -> emailTemplateService.managerReviewDone(
              user.getFullName(),
              cycleName);
      case APPRAISAL_APPROVED -> emailTemplateService.appraisalApproved(
              user.getFullName(),
              cycleName);
      default -> buildGenericEmail(user.getFullName(), title, message);
    };
